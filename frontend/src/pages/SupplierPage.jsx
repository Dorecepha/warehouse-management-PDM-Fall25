    ...
    <div className="p-6 bg-white rounded-xl shadow-lg space-y-6">
      ...
      <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage your product suppliers</p>
        </div>
      ...
      {/* Table Section */}
      {isLoading ? (
        <div className="py-12 text-center text-slate-500 bg-slate-50 rounded-lg border border-dashed border-slate-200">Loading suppliers...</div>
      ) : isError ? (
        <div className="p-4 text-red-700 bg-red-50 rounded-lg border border-red-100">{error.message}</div>
      ) : suppliers.length === 0 ? (
        <div className="py-12 text-center text-slate-500 bg-slate-50 rounded-lg border border-dashed border-slate-200">No suppliers found.</div>
      ) : (
        <div className="overflow-hidden rounded-lg border border-slate-200">
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50">
              <tr>
                {columns.map((column) => (
                  <th key={column.key} className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">
                    {column.header}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="bg-white divide-y divide-slate-200">
              {suppliers.map((supplier) => (
                <tr key={supplier.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600">{supplier.contactInfo}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">{supplier.address}</td>
                  <td className="px-6 py-4 text-sm font-medium flex gap-3">
                    <button onClick={() => navigate('/suppliers/${supplier.id}/edit')} className="text-indigo-600 hover:text-indigo-900">Edit</button>
                    <button onClick={() => handleDelete(supplier.id)} disabled={deleteMutation.isPending} className="text-red-600 hover:text-red-900 disabled:opacity-50">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {totalPages > 1 && (
        <footer className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-100">
          <span className="text-sm text-slate-500">Page {page} of {totalPages} {isFetching && !isLoading ? '(Updating...)' : ''}</span>
